...
class ModalZona extends Component {
	...
	save = () => {
		if (this.state.id_zona > 0) {
			ZonaService.Modificar(this.state).then((response) => {
				this.close();
			...
			ZonaService.Agregar(this.state).then((response) => {
				this.close();
						...
						<Grid container spacing={1}>
							...
							<Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
								...
								<TextField
									...
									value={this.state.id_orientacion}
								>
									<option key={0} value={''}>&nbsp;</option>
									{this.state.listar_orientacion.map(option => (
										<option key={option.id_orientacion} value={option.id_orientacion}>
											{option.orientacion}
										</option>
									))}
								</TextField>
