...
import ModalOrientacion from "./ModalOrientacion";
...
class ListaTabla extends Component {
	...
	eliminar = (item) => {
		let msg = `¿Deseas eliminar la orientación con id: ${item.id_orientacion}?`;
		PopupService.Confirm(['Cancelar', 'Eliminar'], 'warning', CONFIG.titulo_alert_confirm, msg).then((r) => {
			if (r.button === 'Eliminar') {
				OrientacionService.Eliminar(item.id_orientacion).then((response) => {
